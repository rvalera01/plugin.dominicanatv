...
SOURCES = [{"description": ": Canales platalist basado en Livestreamspro :", "title": "Platalist ", "url": "http://platalist.com/PLATATV.XML", "fanart": "http://i.imgur.com/GyK9PGj.jpg", "genre": "TV ONLINE", "date": "23.09.2015", "credits": "Platalist", "thumbnail": "http://i.imgur.com/ZWSRmr1.jpg?1"}]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.Platalist-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Platalist,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Platalist,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(AdryanList,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(AdryanList,Using Urlresolver module.. - ,5000)")
        import urlresolver
